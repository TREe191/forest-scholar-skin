...
export function sanitizeDiagnostic(input,depth=0){
  ...
  for(const [key,value] of Object.entries(input)){
    ...
    else if((k==='at'||k.endsWith('at'))&&/^(?:at|startedat|finishedat|observedat|launcherat|launcherstartedat|listenerfirstseenat|httpfirstsuccessat|browserwebsocketfirstseenat|readyat|attachat)$/.test(k)&&
    ...
    else if(['status','failurestage','mode','requestedmode','configuredappearance','stage','comparisonmode','mismatchreason','reason'].includes(k)&&VALUES.has(value))out[key]=value;
...
export class DiagnosticsExporter{
  ...
  async exportLatest(){
    ...
    const report={schemaVersion:1,appVersion:this.info.version,buildMode:this.info.mode,session:entries[0].name,exportedAt:new Date().toISOString(),privacy:'Allowlisted startup metadata only. Raw logs, text, paths, commands, theme names and user content are excluded.',sources};
