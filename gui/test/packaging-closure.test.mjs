...
import {BUILTIN_IDS,CORE_FILES,buildCore,closeGuiModules,ignoreGuiFile,preparedNodeRoot,projectRoot} from '../scripts/package-resources.mjs';
...
test('detached beta/production resources + userData work with no system Node/PATH', {skip:!prepared?'Run npm run prepare:node first':false},async()=>{
 ...
 try{
  ...
  assert.deepEqual(BUILTIN_IDS,['forest-scholar','phainon']);
  for(const ref of CORE_FILES)assert.ok(resourceNames.includes(ref));
  assert.deepEqual(resourceNames.filter(f=>/^themes\/[^/]+\/theme\.json$/.test(f)).sort(),['themes/forest-scholar/theme.json','themes/phainon/theme.json']);
  assert.ok(resourceNames.includes('node/LICENSE'));
  assert.ok(resourceNames.includes('node/runtime.json'));
  assert.ok(!resourceNames.some(f=>/runtime\/|history\/|preserved\/|user-|universal-demo|universal-dark-test|config\/app\.json|diagnostic|\.log$|\.zip$/i.test(f)));
