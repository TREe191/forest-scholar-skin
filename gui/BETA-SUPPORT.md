...
The build wrapper resolves the current full Git commit before Forge starts, or uses `unavailable` when it cannot be verified. Forge packageAfterCopy writes mode, full commit and seven-character short commit to `src/main/build-mode.json` in the staging directory. Packaged main reads this stamp without accessing `.git` and accepts only beta/production, never runtime environment escalation to development. Unpackaged runs accept THEME_MANAGER_MODE and the wrapper-supplied commit. Missing/invalid packaged mode fails closed; missing/invalid commit remains explicitly `unavailable`.
...
CDP readiness diagnostics include the single listener owner PID when unambiguous, whether it is the activation PID, validated expected-executable evidence, observed activation/child/descendant relationship, HTTP attempt count and timestamps, and only allowlisted HTTP failure categories/status/error codes. Response bodies and exception messages are never exported.
