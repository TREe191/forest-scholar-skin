...
if ($null -eq $session) {
    throw 'No active Forest Scholar Skin session was found.'
}
...
$nodeAction = if ($Action -eq 'Enable') { '--visibility-on' } else { '--visibility-off' }
...
if ($LASTEXITCODE -ne 0) {
    throw "Visibility diagnostic failed with exit code $LASTEXITCODE."
}
